...
public class DBFavouriteProducts extends SQLiteOpenHelper{

    private static final int DB_VERSION = 2;
    private static final String DB_NAME= "dbFavourites";
    private static final String TABLE_FAVOURITES = "tableFavouriteProducts";
    ...
    private static final String CREATE_TABLE = " CREATE TABLE " + TABLE_FAVOURITES + "("
    ...
    public DBFavouriteProducts(Context context){
    ...
    public boolean addProduct(Product product){
        ...
        boolean  rowInserted =  db.insert(TABLE_FAVOURITES,null,values)>0;
        db.close();
        return rowInserted;
    }
    ...
    public  boolean deleteProduct(Product product){
        SQLiteDatabase db = this.getWritableDatabase();
        boolean deleted = db.delete(TABLE_FAVOURITES,
                KEY_NAME + "=?" + " and "  +
                        KEY_BRAND + "=?",
                new String[] {
                        product.getName(),
    ...
    public boolean upDateProduct(Product product){
        ...
        boolean updated =   db.update(TABLE_FAVOURITES, values,
                KEY_NAME + "=?" + " AND "  +
                        KEY_BRAND + "=?",
                new String[] {
                        product.getName(),
    ...
    public  boolean ifExists(Product product) {
        SQLiteDatabase db = this.getReadableDatabase();
        String sql = " SELECT * FROM "+TABLE_FAVOURITES+" WHERE "+KEY_NAME+"=? AND "+KEY_BRAND+"=?";
